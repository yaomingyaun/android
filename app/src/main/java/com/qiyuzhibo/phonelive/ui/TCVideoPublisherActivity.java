...
import com.qiyuzhibo.phonelive.bean.UploadBean;
import com.qiyuzhibo.phonelive.ui.customviews.ErrorDialogFragment;
import com.qiyuzhibo.phonelive.ui.customviews.ProgressPopWinFactory;
...
//    private FixGridLayout mShareType;
    ...
    private void uploadVideoToQiNiu(final String title) {
        ...
        final File mCoverPathFile = new File(mCoverPath);
        final File mVideoPathFile = new File(mVideoPath);
...
//        });
        ...
        PhoneLiveApi.getVideoURL(getUserID(), getUserToken(), mVideoPathFile, new StringCallback() {
            @Override
            ...
            public void onResponse(String response, int id) {
                ...
                try {
                    ...
                    if (jsonObject.getInt("ret") == 200){
                        ...
                        if (null!=data.getString("info")&&!data.getString("info").isEmpty()){
                            videourl = data.getString("info");
                            PhoneLiveApi.getVideoImgURL(getUserID(), getUserToken(), mCoverPathFile, new StringCallback() {
                                @Override
                                public void onError(Call call, Exception e, int id) {
                                    showToast3("发布失败", 0);
                                }
                                ...
                                public void onResponse(String response, int id) {
                                    try {
                                        ...
                                        if (jsonObject.getInt("ret") == 200) {
                                            JSONObject data = jsonObject.getJSONObject("data");
                                            if (null != data.getString("info") && !data.getString("info").isEmpty()) {
                                                imgurl = data.getString("info");
                                                requestUpload(title,new UploadBean(mVideoPathFile,videourl,mCoverPathFile,imgurl));
                                            }
                                        }
